...
| Standards batch 3 | `standards/documentation.md` § One home per fact (one authoritative home, derived files generated and diffed, state written once, one writer per fenced section; zone and authority machinery retired); `standards/integration-and-data.md` (the contract is the source, additive-only compatibility, tolerant readers, one owning system per data set, outbox, idempotent consumers); `standards/collaboration.md` (hand-over finished when the receiver confirms, four task endings `DONE` · `DONE_WITH_CONCERNS` · `BLOCKED` · `NEEDS_DECISION`, decision records with a source, roles for one to a few people); `standards/security.md` § Data classes and model access (`public` · `internal` · `confidential` · `restricted` · `no-ai`, separate from model quality). 12 new MIGRATION-MAP rows, 3 rows extended; 22 external sources opened and re-opened. Candidate rules only; nothing enters the rule register |
