...
# Mã thoát: 0 đạt · 1 vi phạm (kể cả miễn trừ quá hạn) · 2 không đo được.
set -u
...
if [ "$DIR" = --selftest ]; then
  ...
  rm -rf "$t"
  [ "$a$b$c$d$e" = "01012" ] && { echo "selftest OK"; exit 0; }; echo "selftest HỎNG: $a$b$c$d$e (muốn 01012)"; exit 1
fi
[ -d "$DIR" ] || { echo "CHƯA ĐO ĐƯỢC: $DIR không tồn tại"; exit 2; }
...
[ $((vi_pham+qua_han)) -gt 0 ] && exit 1; exit 0
