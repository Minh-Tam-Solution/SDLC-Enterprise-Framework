...
#
# Dùng: kiem-nghiem-phien-ban.sh [--chan] [--selftest]
#   không cờ → đếm, exit 0 (nấc 1 ADVISORY) · --chan → exit 2 nếu khong_khai>0 (nấc 3 MACHINE)
# Mã thoát (21-V7): 0 ĐẠT · 1 KHÔNG ĐO ĐƯỢC (mất SSOT / quét 0 file / tham số lạ) · 2 VI PHẠM.
# Dòng cuối stdout = nhãn: result=pass|insufficient_evidence|violation gate=kiem-nghiem-phien-ban reason=<slug>
set -uo pipefail
CHAN=0; TU_THU=0
nhan() { echo "result=$1 gate=kiem-nghiem-phien-ban reason=$2"; }
for a in "$@"; do case $a in --chan) CHAN=1;; --selftest) TU_THU=1;;
  *) echo "KHÔNG ĐO ĐƯỢC: tham số lạ $a"; nhan insufficient_evidence tham_so_la; exit 1;; esac; done

if [ $TU_THU = 1 ]; then
  # Fixture cục bộ: chép chính script vào cây tạm (ROOT = ../.. của script) — không đụng repo thật.
  t=$(mktemp -d); S="$t/05-Templates-Tools/07-Scripts/$(basename "$0")"
  mkdir -p "$(dirname "$S")" "$t/d"; cp "$0" "$S"
  kq() { local o rc; o=$(bash "$S" "$@"); rc=$?; echo "$rc|$(echo "$o" | grep -oE 'khai_cu=[0-9]+  khong_khai=[0-9]+')|${o##*$'\n'}"; }
  printf '# r\n**Version**: 9.9.9\n' > "$t/README.md"
  a=$(kq --chan)                                                            # chỉ có file miễn ⇒ quét 0 ⇒ 1 (G1)
  printf '# a\n**SDLC Framework Version**: 9.9.9\n' > "$t/d/moi.md";     b=$(kq --chan)   # khai đúng SSOT ⇒ 0
  printf -- '---\nsdlc_framework: "1.0.0"\n---\n' > "$t/d/cu.md";       c=$(kq --chan)   # khai_cu KHÔNG phải lỗi ⇒ 0
  printf '# khong khai\n' > "$t/d/khong.md";                             d=$(kq --chan)   # thiếu trường ⇒ 2
  e=$(kq)                                                                   # nấc 1: chỉ đếm ⇒ 0
  rm "$t/README.md";                                                        f=$(kq --chan)   # mất SSOT ⇒ 1
  rm -rf "$t"
  G=gate=kiem-nghiem-phien-ban
...
1||result=insufficient_evidence $G reason=thieu_ssot_readme"
  got=$(printf '%s\n' "$a" "$b" "$c" "$d" "$e" "$f")
  [ "$got" = "$want" ] && { echo "selftest OK (6 ca)"; nhan pass selftest_ok; exit 0; }
