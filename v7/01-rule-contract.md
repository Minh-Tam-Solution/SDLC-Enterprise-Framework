...
- **C21-5** `scripts/check-version-declared.sh` (which framework version was a document verified against) — `ADVISORY` at step 1. It has `--selftest` and `--block` returning `2`; it still needs a `count=` line and a **deadline published in advance** (§4). Adding it earlier would make C21-4 flag it as mis-declared.
