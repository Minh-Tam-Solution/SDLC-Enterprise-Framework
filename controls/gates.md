...
- Health red after restart ⇒ **automatic rollback only when `migration` is `none`, or `expand` and the migration itself declares `migration-class: expand`** (machine and author agree; while MIG-1 is `ADVISORY` the machine alone does not authorise a rollback). Rolling code back onto a contracted schema — a dropped column, a changed type, a removed enum value — breaks more than it fixes.
...
- Any other case — `contract`, `unknown`, or expand without the author's declaration — and health red ⇒ stop, alert loudly, follow the human runbook.
