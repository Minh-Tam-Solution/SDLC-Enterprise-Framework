...
| no new secrets | FRAMEWORK_REPO (adopters: PRODUCT_CI) | no secret-shaped value in lines a change adds; false-positive exemptions are read from the base commit, need owner, reason and a 90-day expiry; blocks | `scripts/rule-no-new-secrets.sh` |
...
`migration` is `none`, `expand`, `unknown` or `contract`, as classified by `scripts/check-migration-class.sh` from the migration's statements — not as declared by its author. A statement the classifier does not recognise makes the migration `unknown`, never `expand`.
...
- Health red after restart ⇒ **automatic rollback only when `migration=none`.** While MIG-1 is `ADVISORY` its classification decides nothing in production: an advisory detector must not change what is safe, and a migration's own `migration-class` label is no second check — the same agent may have written both. `expand` may join this rule only after MIG-1 is promoted on a labelled corpus that measures its false-expand rate. Rolling code back onto a contracted schema — a dropped column, a changed type, a removed enum value — breaks more than it fixes.
...
- Any deploy with a migration and health red ⇒ stop, alert loudly, follow the human runbook.
