...
rows=$(find . -name '*.md' -not -path './10-Archive/*' -not -path './.git/*' -exec awk '
  ...
  { n=split($0,c,"|"); for(i=2;i<n;i++) gsub(/^[[:space:]]+|[[:space:]]+$/,"",c[i])
    if (!b) { b=1; split("",k); for(i=2;i<n;i++) k[c[i]]=i; ok=("class" in k)&&("cmd" in k)&&("burn_case" in k); print "__TABLE__"; next }
...
[ "$tables" -gt 0 ] || { echo "CANNOT MEASURE: no ## Rules v7 table under $GOC"; label insufficient_evidence no_table; exit 1; }
