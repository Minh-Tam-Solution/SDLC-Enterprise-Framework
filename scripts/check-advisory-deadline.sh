#!/usr/bin/env bash
# C21-4 (G4): every class=ADVISORY row in a `## Rule register` table (controls/rule-contract.md; the legacy heading `## Rules v7` is still read until 2026-12-31) must carry a COUNTER + a DEADLINE.
...
#   --goc missing / no rule-register table at all ⇒ 1 · tables exist but 0 ADVISORY rows ⇒ 0 (advisory=0).
...
if [ $SELFTEST = 1 ]; then
  ...
  # legacy heading on purpose: `## Rules v7` must still be read until 2026-12-31
...
# Rows of every `## Rule register` (or legacy `## Rules v7`) table in any .md (skip archive, .git). Columns located by header NAME —
...
rows=$(find . -name '*.md' -not -path './archive/*' -not -path './.git/*' -exec awk '
  FNR==1 {t=0; b=0}
  /^## / {t=($0 ~ /^## (Rule register|Rules v7)[[:space:]]*$/); b=0; next}
  !/^\|/ {b=0; next}
  !t || /^\|[-: |]+\|$/ {next}
...
[ $((tables+bad_header)) -gt 0 ] || { echo "CANNOT MEASURE: no ## Rule register table under $GOC"; label insufficient_evidence no_table; exit 1; }
