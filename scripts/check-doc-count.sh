...
#   Counts tracked *.md files outside 10-Archive/ and templates/ (git index; untracked files do not count).
#   ADVISORY above 40: prints a warning, exit 0. MACHINE above 60: exit 2. Thresholds published in v7/03-gates.md.
...
if [ $SELFTEST = 1 ]; then
  ...
  mk() { # mk DIR live_docs archived_and_template_docs
    mkdir -p "$1/v7" "$1/10-Archive/v6" "$1/templates" && git -C "$1" init -q || return 1
    for i in $(seq 1 "$2"); do : > "$1/v7/d$i.md"; done
    for i in $(seq 1 "$3"); do : > "$1/10-Archive/v6/a$i.md"; : > "$1/templates/t$i.md"; done
    : > "$1/v7/not-a-doc.txt"; git -C "$1" add -A
  }
  ...
  bash "$0" --root "$t/green/v7" > "$t/out" 2>&1; e=$?   # a subdirectory is not a repo root
  got="$a$b$c$d$e $ga$gb"
...
list=$(git -C "$ROOT" ls-files -z -- '*.md' ':(exclude)10-Archive/' ':(exclude)templates/' | tr '\0' '\n') \
