#!/usr/bin/env bash
# Rule DOC-2: every live document names an Owner, a Consumer and a Review-by date, and the date has not passed.
...
# Fields are read from the first 30 lines, as `**Owner**: <value>` (a value ends at " · " or end of line):
...
if [ $SELFTEST = 1 ]; then
  ...
  printf "# Good\n\n$ok" > "$t/r/core/good.md"
  printf '# Inline\n\n**Version**: 1.0.0 · **Owner**: @m · **Consumer**: gate authors · **Review by**: 2026-12-31\n' > "$t/r/core/inline.md"
  git -C "$t/r" add -A
  ...
  got="$a $b $n $c"
  [ "$got" = "0 2 count=4 1" ] && { echo "selftest OK"; label pass selftest_ok; exit 0; }
  echo "selftest BROKEN: got=[$got] want=[0 2 count=4 1]"; cat "$t/out"; label insufficient_evidence selftest_broken; exit 1
fi
...
out=$(cd "$ROOT" && printf '%s\n' "$docs" | TODAY="$today" perl -e '
  ...
  while (my $f = <STDIN>) {
    ...
    while (my $l = <$fh>) {
      last if ++$n > 30;
      for my $k ("Owner", "Consumer", "Review by") {
...
[ "$n" -gt 0 ] && { label violation "docs_$n" "add_Owner_Consumer_Review-by_to_each_listed_doc_or_review_it_and_move_the_date"; exit 2; }
