...
- **Stage gates are decision points; [`03-gates.md`](03-gates.md) gates are scripts.** A stage gate is passed when its evidence exists and, where the tier requires it, the right person has signed. Every piece of evidence that a script can check should be checked by one.
- Folder layout, required files per tier and naming: [`08-project-structure.md`](08-project-structure.md) and [`09-documentation-standards.md`](09-documentation-standards.md).

## Exit evidence per stage

"Minimum" applies to every tier that requires the stage. "PROFESSIONAL+" is added on top for PROFESSIONAL and ENTERPRISE ([`02-tiers.md`](02-tiers.md)).
...
| 02 | ADRs for every decision that is expensive to reverse · one architecture diagram | security architecture · risk-floor paths declared ([`risk-floor-paths.md`](risk-floor-paths.md)) |
...
| 04 | code merged · tests for the changed behaviour · README runs as written | CI green on the merge commit · fresh-context review artifact bound to the commit ([`02-tiers.md`](02-tiers.md)) |
...
**Negative evidence is still evidence.** "No integrations" or "no regulated data" is written as a one-line document in the stage folder, with the date and the author. An empty folder cannot be told apart from forgotten work.

## Which stages each tier requires
...
"Optional" means the folder may be absent. Once a folder exists, its documents follow the same standards as required ones.
...
At the end of every sprint the sprint file ([`08-project-structure.md`](08-project-structure.md#planning-files)) states, for each stage touched: what exited, what was re-opened, and what evidence is still missing. A sprint closes when the file is updated, not when the calendar says so.
